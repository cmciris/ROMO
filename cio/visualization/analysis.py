    ...
    for file in files:
        if file.endswith("_scores.npy"):
            ...
            with open(os.path.join(logs_dir, file[:-11] + "_info.log"), "r") as info:
                ...
                while True:
                    line = info.readline().strip()
                    if (line.startswith("(train/loss") and "mean" in line) or (line.startswith("(train/loss") and "value" in line):
                        one_seed.append(float(line.split(",")[0].split(":")[-1]))
                        break
                while True:
                    line = info.readline().strip()
                    if (line.startswith("(train/loss") and "mean" in line) or (line.startswith("(validate/loss") and "mean" in line):
                        one_seed.append(float(line.split(",")[0].split(":")[-1]))
                        break
