...
class StaticGraphTask(object):
    def __init__(self, data_dir, ** task_kwargs) -> None:
        """
        An interface to a static-graph task which includes a validation set and a non differentiable score functionn
        ...
        **task_kwargs: dict
        ...
        if task_kwargs.get('relabel', False):
            ...
            self.aux = np.load(os.path.join(data_dir, 'relabel/aux.npy')).astype(np.float32)
            ...
            self.aux = np.load(os.path.join(data_dir, 'aux.npy')).astype(np.float32)
            self.y = np.load(os.path.join(data_dir, 'y.npy')).astype(np.float32)

        if self.max_samples is not None:
            self.x = self.x[:self.max_samples]
            self.aux = self.aux[:self.max_samples]
            self.y = self.y[:self.max_samples]
        self.is_discrete = False  # for cio
        ...
        self.x, self.aux, self.y = self.clip_outliers(self.x, self.aux, self.y, k=1000)
    ...
    @staticmethod
    def clip_outliers(x, aux, y, k=10) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """
        clip outliers on cpu, delete max k samples & min k samples.
        """
        assert x.shape[0] == aux.shape[0] == y.shape[0]
        ...
        aux = np.delete(aux, indices_clip, axis=0)
    ...
    def map_normalize_aux(self):
        """a function that standardizes the prediction values 'aux' to have
        zero empirical mean and unit empirical variance in the dataset

        """
        if ~self.is_normalize_aux:
            self.mu_aux = np.mean(self.aux, axis=0, keepdims=True).astype(np.float32)
            self.aux = self.aux - self.mu_aux
            self.st_aux = np.std(self.aux, axis=0, keepdims=True).astype(np.float32).clip(1e-6, 1e9)
            self.aux = self.aux / self.st_aux

            self.is_normalize_aux = True
    ...
    def denormalize_aux_numpy(self, aux):
        """a function that un-standardizes the auxiliary values 'aux' which have
        zero empirical mean and unit empirical variance
        ...
        """
        if self.is_normalize_aux:
            aux = aux * self.st_aux + self.mu_aux
    ...
    def denormalize_aux(self, aux):
        ...
        """
        if self.is_normalize_aux:
            st_aux = torch.tensor(self.st_aux, device=aux.device)
            mu_aux = torch.tensor(self.mu_aux, device=aux.device)
            aux = aux * st_aux + mu_aux
    ...
    def predict_numpy(self, aux):
        """a function that accepts a batch of predicted auxiliary labels 'aux' as input and
        for each auiliary value computes a value 'y' which corresponds
        to the groundtruth score in a model-based optimization problem

        Arguments:

        aux_batch: np.ndarray
        ...
        y = - np.sum(np.var(aux.reshape((batch_size, aux.shape[1] // 2, 2)), axis=-2), axis=-1, keepdims=True)
        if self.is_score_scale:
            range_interval = self.score_range[1] - self.score_range[0]
            range_low = self.score_range[0]
            y = ((y - self.score_min) * 1.) / (self.score_max - self.score_min) * range_interval + range_low
            
        return y

    def predict(self, aux):
        """a function that accepts a batch of predicted auxiliary labels 'aux' as input and
        for each auiliary value computes a value 'y' which corresponds
        to the groundtruth score in a model-based optimization problem
        ...
        # pdb.set_trace()
        batch_size = aux.shape[0]
        y = - torch.sum(torch.var(aux.reshape((batch_size, aux.shape[1] // 2, 2)), dim=-2), dim=-1, keepdim=True)
        if self.is_score_scale:
            range_interval = self.score_range[1] - self.score_range[0]
            range_low = self.score_range[0]
            y = ((y - self.score_min) * 1.) / (self.score_max - self.score_min) * range_interval + range_low
...
class CIODataSet(Dataset):
    def __init__(self, tuples) -> None:
        self.x = tuples[0]
        self.aux = tuples[1]
        self.y = tuples[2]

        assert len(self.x) == len(self.aux) == len(self.y)
...
def build_pipeline(x, aux, y, val_size=200, train_size=800, batch_size=128, bootstraps=0, bootstraps_noise=None, buffer=None):
    ...
    Args:
        ...
        aux: torch.Tensor
            a tensor containing auxiliary values from a model-based optimization
            dataset, typically taken from task.aux
        y: torch.Tensor
    ...
    train_inputs = [x[val_size:train_end], aux[val_size:train_end], y[val_size:train_end]]
    validate_inputs = [x[:val_size], aux[:val_size], y[:val_size]]
    ...
    if bootstraps > 0:
        bootstraps_training_dataset = [CIODataSet([bootstraps_train_inputs[0][b], bootstraps_train_inputs[1][b], bootstraps_train_inputs[2][b]]) for b in range(bootstraps)]
        validation_dataset = CIODataSet(validate_inputs)
        ...
        training_dataset = CIODataSet(train_inputs)
        validation_dataset = CIODataSet(validate_inputs)
...
def build_rim_pipeline(x, aux, y, pool_size=800, val_size=200, train_size=800, batch_size=128, bootstraps=0, bootstraps_noise=None, buffer=None):
    """Split a model-based optimization dataset consisting of a set of design
    values x and prediction values y into a training and validation set,
    supporting bootstrapping and importance weighting
    Args:
    ...
    train_inputs = [x[val_size + pool_size : train_end], aux[val_size + pool_size : train_end], y[val_size + pool_size : train_end]]
    pool_inputs = [x[val_size : val_size + pool_size], aux[val_size : val_size + pool_size], y[val_size : val_size + pool_size]]
    validate_inputs = [x[:val_size], aux[:val_size], y[:val_size]]
